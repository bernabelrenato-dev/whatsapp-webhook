...
async function main() {
  try {
    ...
    groups.forEach(group => {
      // Filter out any dummy debugging block
      group.blocks = group.blocks.filter(b => b.id !== 'dummy-text-input-id');
        ...
        if (block.id === blockWebhookId) {
          ...
          block.options = {
            ...
            responseVariableMapping: [
              { id: 'mapping-message-id', bodyPath: 'data.message', variableId: 't7t3kuz4dnwc5tatsonvjuto' },
              { id: 'mapping-image-id', bodyPath: 'data.imageUrl', variableId: '869oi7estszj6f951dh643pm' }
            ],
            webhook: {
...
`;
    ...
    const sqlPath = path.join(__dirname, 'apply-v6-inline-webhook-final.sql');
    fs.writeFileSync(sqlPath, sqlContent, 'utf8');
    
    // 5. Copy and execute in DB
    execSync(`docker cp ${sqlPath} typebot-db:/tmp/apply-v6-inline-webhook-final.sql`);
    execSync(`docker exec typebot-db psql -U postgres -d typebot -f /tmp/apply-v6-inline-webhook-final.sql`);
    console.log('Database updated successfully to final structure!');
