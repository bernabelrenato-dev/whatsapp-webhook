...
async function auditExcel(fileName) {
  ...
  for (const sheetName of workbook.SheetNames) {
    ...
    // Helper function to check if row is a new product
    function checkNewProduct(row) {
      let hasCode = false;
      let codeStr = '';
      ...
      if (hasCode) return { isProduct: true, code: codeStr, name: nameStr };
      if (hasName) return { isProduct: true, code: '', name: nameStr };
    ...
    for (let i = headerRowIndex + 1; i < rows.length; i++) {
      ...
      const prodCheck = checkNewProduct(row);
      if (prodCheck.isProduct) {
        productsCount++;
        if (sampleProducts.length < 3) {
          sampleProducts.push({
            code: prodCheck.code || 'N/A',
            name: prodCheck.name.slice(0, 80),
...
async function run() {
  ...
  fs.writeFileSync('scripts/detailed_audit_results_v3.json', JSON.stringify(results, null, 2), 'utf-8');
  console.log('Done! Wrote v3 audit results to scripts/detailed_audit_results_v3.json');
}
